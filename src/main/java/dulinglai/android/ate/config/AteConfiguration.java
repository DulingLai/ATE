...
public class AteConfiguration {
    ...
    public AteConfiguration() { }

    public void merge(AteConfiguration config) {
        if (config != null) {
            this.oneComponentAtATime = config.oneComponentAtATime;

            this.callbackConfig.merge(config.callbackConfig);
            this.sourceSinkConfig.merge(config.sourceSinkConfig);
            this.iccConfig.merge(config.iccConfig);
            this.analysisFileConfig.merge(config.analysisFileConfig);

            this.sootIntegrationMode = config.sootIntegrationMode;
            this.mergeDexFiles = config.mergeDexFiles;
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        AteConfiguration other = (AteConfiguration) obj;
