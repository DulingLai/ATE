...
//                config.getAnalysisFileConfig().getOutputFile(), manifest.getPackageName());
        ...
        LayoutFileParser layoutFileParser = new LayoutFileParser(manifest.getPackageName(), resourceValueProvider);
...
//        Logger.info("[{}] ... End analyzing the class files ...", CLASS_ANALYZER);
        ...
        ResultWriter.writeActivityListToFile(manifest.getActivityNodeList(), manifest.getPackageName(),
...
//        }
    ...
    private void addWidgetToEdge() {
        for (TransitionEdge edge : widgetEdgeMap.keySet()) {
            AbstractComponentNode src = edge.getSrcComp();
            AbstractComponentNode tgt = edge.getTgtComp();
...
//    }
    ...
    private List<IccIdentifier> buildComponentTransitionGraph(List<IccLink> iccLinks, ProcessManifest manifest) {
        ...
        List<ActivityNode> activityNodeList = manifest.getActivityNodeList();
        List<ServiceNode> serviceNodeList = manifest.getServiceNodeList();
        List<BroadcastReceiverNode> receiverNodeList = manifest.getReceiverNodeList();
        List<ContentProviderNode> providerNodeList = manifest.getProviderNodeList();
...
//    }
    ...
    private void combineClassAndXMLData(MultiMap<String, AndroidLayoutControl> userControls) {
        ...
        for (SootClass layoutClass : layoutClasses.keySet()) {
            for (ActivityNode activityNode : manifest.getActivityNodeList()) {
    ...
    private ClickWidgetNode createClickWidgetForUserControl(GenericLayoutControl usercontrol) {
        ...
        if (clickListener!=null) {
            ...
            if (resId!=-1) {
                String resString = resourceValueProvider.getLayoutResourceString(resId);
    ...
    private boolean collectXmlBasedCallbackMethods(LayoutFileParser lfp, AbstractJimpleAnalyzer jimpleClass) {
        ...
        for (final SootClass callbackClass : jimpleClass.getLayoutClasses().keySet()) {
            ...
            for (Integer classId : classIds) {
                final String layoutFileName = resourceValueProvider.getLayoutResourceString(classId);
