...
//    }
    ...
    public static void loadConfigFile(String configFilePath, AteConfiguration config){
        ...
        try {
            ...
            String outputPathConfig = config.getAnalysisFileConfig().getAndroidPlatformDir();
            ...
            if (outputPathConfig == null || outputPathConfig.isEmpty()){
                if (outputPathFile!=null)
                    config.getAnalysisFileConfig().setOutputFile(outputPathFile);
                else {
            ...
            String androidSdkConfig = config.getAnalysisFileConfig().getAndroidPlatformDir();
            // use the android sdk path in the config file if the user does not specifies a path for androidSdk
            if (androidSdkConfig == null || androidSdkConfig.isEmpty()){
                if (androidSdkFile!=null)
                    config.getAnalysisFileConfig().setAndroidPlatformDir(androidSdkFile);
                else {
            ...
            int androidApiConfig = config.getAnalysisFileConfig().getTargetApiLevel();
            // use the android sdk path in the config file if the user does not specifies a path for androidSdk
            if (androidApiFile==null) {
                if (androidApiConfig!=-1) {
                    ...
                    config.getAnalysisFileConfig().setTargetApiLevel(Integer.parseInt(androidApiFile));
            }
